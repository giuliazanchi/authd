...
func (m *Manager) UpdateUser(u UserInfo) (err error) {
	...
	if errors.Is(err, cache.NoDataFoundError{}) {
		...
		u.UID = oldUser.UID
		oldGroups, err = m.cache.GroupsOfUser(oldUser.UID)
		if err != nil {
	...
	if err := m.cache.UpdateUserEntry(userDB, authdGroups); err != nil {
		return err
	}

	// Update local groups.
	if err := localentries.Update(u.Name, localGroups); err != nil {
